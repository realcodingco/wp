...
function appendSubscribeTable(target, data) { 
    ...
    const appdx = `* 7-day money back guarantee. For refund inquiries, please contact <font color=#2b68a7>${adminEmail}</font>.<br>
* If you're already subscribed, your final payment may vary depending on whether the subscription plan you choose is a renewal or upgrade, so please check at checkout.<br>
* To extend your subscription, make an additional subscription payment before it expires.`;
...
function requsetGlobalPay(json) {
    ...
    obj.resultUrl = 'https://www.realcoding.co/cest/payple-callback';
    obj.isDirect = "";
    obj.serviceDefine = JSON.stringify(json); //결제정보

    axios.post('https://www.realcoding.co/cest/payple-auth') // 토큰발급 요청
        .then(res => {

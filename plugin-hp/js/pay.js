...
isLogin(function(result) { 
    user = result;
    const payType = new URLSearchParams(location.search).get('type') || localStorage.getItem('pay_plan');
    const state = new URLSearchParams(location.search).get('state') || localStorage.getItem('pay_state');
    
    if(!payType) {
        toastr.error('Please advance from the beginning again', 'The wrong approach');
        return;
    }
